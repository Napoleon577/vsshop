...
public class DownDemo {
    ...
    public static String getRealDownloadUrl(String downloadUrl, String cookie,String sourceId) throws IOException {
        ...
        try (Response response = client.newCall(request).execute()) {
            if (response.isSuccessful() && response.body() != null) {
                ...
                System.out.println("没有解压"+ responseStream);

                if ("gzip".equalsIgnoreCase(encoding)) {
                    responseStream = new GZIPInputStream(responseStream);
                    System.out.println("解压"+ responseStream);
    ...
    // 下载文件并保存到D盘的方法
    public static void downloadFile(String fileUrl) throws IOException, URISyntaxException {
        if (Desktop.isDesktopSupported()) {
            Desktop desktop = Desktop.getDesktop();
            desktop.browse(new URI(fileUrl)); // 打开默认浏览器并访问下载链接
        } else {
    ...
    public static void main(String[] args) throws IOException, URISyntaxException {
        final String FILES_DIRECTORY = "D:\\Files\\"; // 文件保存目录
        // 示例下载地址
        String resourceUrl = "https://download.csdn.net/download/snowjake/88093806?ops_request_misc=%257B%2522request%255Fid%2522%253A%252221AF6032-5679-4F03-B13F-14DAC01B609F%2522%252C%2522scm%2522%253A%252220140713.130102334.pc%255Fdownload.%2522%257D&request_id=21AF6032-5679-4F03-B13F-14DAC01B609F&biz_id=1&utm_medium=distribute.pc_search_result.none-task-download-2~download~first_rank_ecpm_v1~rank_v31_ecpm-1-88093806-null-null.269^v2^control&utm_term=OCR%E8%AF%86%E5%88%AB%E6%A8%A1%E5%9E%8B&spm=1018.2226.3001.4451.2";
